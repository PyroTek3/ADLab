...
.EXAMPLE
PS>.\Invoke-ADLabBuildOut.ps1 -Domain activedirectory.lab 
...
Function Add-ComputerAccountstoAdmins
 {
    Param
     (
        [Parameter(Mandatory=$true)][string]$Domain,
        [Parameter(Mandatory=$true)][int]$NumberOfComputerAccountsInAdmin,
        [Parameter(Mandatory=$true)][string]$ComputerOU,
        [Parameter(Mandatory=$true)][array]$ADAdminGroups
      )

    $DomainDC = (Get-ADDomainController -Discover -DomainName $Domain).Name
    $DomainInfo = Get-ADDomain -Server $DomainDC

    $ComputerPathDN = $ComputerOU + ',' + $DomainInfo.DistinguishedName

    [array]$ComputerArray = Get-ADComputer -Filter * -SearchBase $ComputerPathDN

    [int]$DoWhileAddComputerLoop = 0
    DO
     {
        $DoWhileAddComputerLoop++
    ...
    WHILE
     ( $DoWhileAddComputerLoop -lt $NumberOfComputerAccountsInAdmin ) 
 }

Function Add-HybridServiceAccounts
 {
    Param
     (
        [Parameter(Mandatory=$true)][string]$Domain,
        [Parameter(Mandatory=$true)][string]$ServerOU,
        [Parameter(Mandatory=$true)][string]$ServiceAccountOU
      )
    ...
    $DomainDC = (Get-ADDomainController -Discover -DomainName $Domain).Name
    $DomainInfo = Get-ADDomain -Server $DomainDC
    $DefaultUserOUDN = $DomainInfo.UsersContainer

    $ServerPathDN = $ServerOU + ',' + $DomainInfo.DistinguishedName
    $ServiceAccountPathDN = $ServiceAccountOU + ',' + $DomainInfo.DistinguishedName

    Write-Host "Creating Azure AD Connect server computer account in $Domain ..." -ForegroundColor Cyan
    $AzureADConnectComputerName = 'AZURESYNC' + $(Get-Random -min 10 -max 99)
    New-ADComputer -Name $AzureADConnectComputerName -SamAccountName $AzureADConnectComputerName -Path $ServerPathDN -Server $DomainDC

    Write-Host "Creating Azure AD Connect Service Account in $Domain ..." -ForegroundColor Cyan
    $AzureADTenantID = 'trd' + (Get-Random -min 111111111 -max 999999999)
    $AzureADTenantDomainName = $($DomainInfo.DNSRoot).ToLower() + '.onmicrosoft.com'
    $AzureADConnectDescription = "Account created by the Windows Azure Active Directory Sync tool with installation identifier '" + $AzureADTenantID + "' running on computer '" + $AzureADConnectComputerName + "' configured to synchronize to tenant '" + $AzureADTenantDomainName + "'. This account must have directory replication permissions in the local Active Directory and write permission on certain attributes to enable Hybrid Deployment." 
    $AzureADConnectUserID = "MSOL_" + $(Get-Random -min 111111111 -max 999999999)
    $Password = ConvertTo-SecureString -AsPlainText $([System.Web.Security.Membership]::GeneratePassword(20,2)) -Force
    New-ADUser –name $AzureADConnectUserID -SamAccountName $AzureADConnectUserID -Path $ServiceAccountPathDN -DisplayName $AzureADConnectUserID -Description $AzureADConnectDescription -Enabled $True -AccountPassword $Password -PasswordNeverExpires $PasswordNeverExpiresValue -Server $DomainDC 
    
    ## Create Azure AD Seamless Single-Signon (Azure AD SSO) Computer account
    Write-Host "Creating Azure AD Seamless Single-Signon (Azure AD SSO) Computer account in $Domain ..." -ForegroundColor Cyan
    New-ADComputer -Name 'AZUREADSSO' -SamAccountName 'AZUREADSSO' -Path $DefaultUserOUDN -Server $DomainDC
 ...
 Function Create-ADSite
  {
    ...
    DO
     {
        $DoWhileLoopCount++
        $SiteName = ($CitiesNameArray | Get-Random -Count 1)
...
IF ($InvokeRandomizeAdmins -eq $True) 
  {
    Invoke-RandomizeAdmins -Domain $Domain -AdminOU 'OU=Accounts,OU=AD Administration' -ADAdminGroups @('Administrators','Account Operators','Backup Operators','Cert Publishers','DNSAdmins','Domain Admins','Print Operators','Server Operators')
  }

IF ($InvokeRandomizeServiceAccountAdmins -eq $True) 
  {
    Invoke-RandomizeServiceAccountAdmins -Domain $Domain -MaxServiceAccountsInAGroup 10 -ServiceAccountOU 'OU=Service Accounts,OU=Enterprise Services' -ADAdminGroups @('Administrators','Account Operators','Backup Operators','Cert Publishers','DNSAdmins','Domain Admins','Print Operators','Server Operators')
  }
...
IF ($AddComputerAccountstoAdmins -eq $True) 
  {
    Add-ComputerAccountstoAdmins -Domain $Domain -NumberOfComputerAccountsInAdmin 5  -ComputerOU 'OU=Servers,OU=Enterprise Services' -ADAdminGroups @('Administrators','Account Operators','Backup Operators','DNSAdmins','Domain Admins','Print Operators','Server Operators')
  }
